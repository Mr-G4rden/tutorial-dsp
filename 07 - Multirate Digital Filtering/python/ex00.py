...
w, Hf_fir = signal.freqz(b=fir_num, a=1,
                         worN=nFFT,
                         whole=False)
w, Xf = signal.freqz(b=x,a=1,
                    worN=nFFT,
                    whole=False)
w, Xf_u = signal.freqz(b=xu,a=1,
                      worN=nFFT,
                      whole=False)
w, Yf = signal.freqz(b=y,a=1,
                    worN=nFFT,
                    whole=False)
...
Xf = 20*np.log10(np.abs(Xf)/nFFT)
Xf_u = 20*np.log10(np.abs(Xf_u)/nFFT)
Yf = 20*np.log10(np.abs(Yf)/nFFT)
